				...
				<c:forEach var="epc" items="${epclist}">
					<c:if test="${epc.EPC_PARENT_ID == 0}">
						<tr>
							...
							<td>${epc.EPC_NAME}</td>
							<td class="w1 c"><a href="toProductClassUpdate?id=${epc.EPC_ID}">修改</a> <a href="productClassDel?id=${epc.EPC_ID}">删除</a></td>
						</tr>
						<c:forEach var="epc2" items="${epclist}">
							<c:if test="${epc.EPC_ID==epc2.EPC_PARENT_ID&&epc2.EPC_ID!=epc2.EPC_PARENT_ID}">
								<tr>
									...
									<td class="w1 c"><a href="toProductClassUpdate?id=${epc2.EPC_ID}">修改</a> <a href="javascript:productClassDel(${epc2.EPC_ID})">删除</a></td>
								</tr>
							</c:if>
						</c:forEach>
					</c:if>
